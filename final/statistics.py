...
def use_stat_for_spectr(y1, y2):
    ...
    for i in indexes:
        ...
        [hist1, edges1] = np.histogram(a=s1[i], bins=450)
        [hist2, edges2] = np.histogram(a=s2[i], bins=450)
...
def regression(chi, student, mann):
    ...
    inside = 0
    if (chi > 0.78) and (student > 0.78) and (mann > 0.78):
        inside = 1
